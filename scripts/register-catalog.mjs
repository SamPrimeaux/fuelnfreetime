...
const [sourceDatabase,targetDatabase]=process.argv.slice(2);
if(!sourceDatabase||!targetDatabase)throw new Error('Usage: register-catalog.mjs <repository-registry-database-id> <catalog-database-id>');
...
const manifest=JSON.parse(fs.readFileSync(path.join(root,'catalog-products.json')));
for(const p of manifest.products){
 if(!fs.existsSync(path.join(root,p.canonical_path)))throw new Error('Canonical source missing: '+p.slug);
 await query(targetDatabase,`INSERT INTO agentsam_products(slug,name,kind,status,repository_id,canonical_path,package_name,version,metadata) VALUES(?,?,?,'wired',?,?,?,?,?) ON CONFLICT(slug) DO UPDATE SET name=excluded.name,kind=excluded.kind,repository_id=excluded.repository_id,canonical_path=excluded.canonical_path,package_name=excluded.package_name,version=excluded.version WHERE json_extract(agentsam_products.metadata,'$.account_id')=json_extract(excluded.metadata,'$.account_id')`,[p.slug,p.name,p.kind,repo.id,p.canonical_path,p.package_name,p.version,JSON.stringify({origin:'developer',account_id:repo.account_id})]);
}
...
console.log(`Registered ${manifest.products.length} source-verified products and ${manifest.relationships.length} relationships`);
