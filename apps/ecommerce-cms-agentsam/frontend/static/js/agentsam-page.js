...
async function handleFilesSelected(fileList) {
  ...
  if (!files.length || !attachmentController) return;
  try { await attachmentController.add(files); }
  catch (error) { appendBubble("assistant", error.message || "Upload failed."); }
  pendingAttachments = attachmentController.items;
...
function renderAttachmentTray() {
  ...
  pendingAttachments.forEach((attachment) => {
    ...
    remove.addEventListener("click", () => {
      attachmentController.remove(attachment.id);
      pendingAttachments = attachmentController.items;
      renderAttachmentTray();
    });
...
async function boot() {
  const {attachCapabilityMenu,createAttachmentController} = await import('/admin/workbench/index.js');
  attachmentController = createAttachmentController({maxFiles:MAX_ATTACHMENTS,maxBytes:MAX_FILE_BYTES,upload:async file => {
    const uploaded = await uploadFile(file);
    return {id:uploaded.attachment_id,attachment_id:uploaded.attachment_id,name:uploaded.file_name,mime_type:uploaded.mime_type,kind:uploaded.kind,size_bytes:uploaded.file_size_bytes,preview_url:uploaded.kind === 'image' ? uploaded.preview_url : null,url:uploaded.preview_url};
  }});
  bindUi();
  const composerInput = $('agentsam-page-input');
  if (composerInput) attachCapabilityMenu(composerInput, {
    list: () => mcpServers.filter(server => server.connected).map(server => ({id:server.slug,label:server.display_name || server.slug})),
    select: item => { activeConnections.add(item.id); renderConnectionPills(); }
  });
  try {
