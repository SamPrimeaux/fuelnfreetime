...
Frontend:               apps/ecommerce-cms-agentsam/frontend/ (React SPA + static admin + shell/annotation adapter)
Backend:                apps/ecommerce-cms-agentsam/backend/ (Worker + admin/CMS/commerce/AgentSam runtime)
Frontend deploy:        npm run build:admin assembles app/theme/package sources into dist/assets; public/admin is retired
...
2. Read runtime contract before commerce/Stripe/CMS/AgentSam changes (docs/RUNTIME-CONTRACTS-*.md in client repo).
3. Runtime source authority is apps/ecommerce-cms-agentsam/{frontend,backend}. Do not restore implementation under admin-ui/, app/, src/, or public/admin/.
...
| File | Purpose | Notes |
|------|---------|-------|
| apps/ecommerce-cms-agentsam/backend/index.js | Worker router | Store + admin + CMS + webhooks |
| apps/ecommerce-cms-agentsam/backend/store/ | Public store runtime | Catalog, checkout, inventory |
| apps/ecommerce-cms-agentsam/backend/admin/ | Admin API runtime | Commerce, media, mail, AgentSam |
| apps/ecommerce-cms-agentsam/frontend/ | Canonical admin frontend | SPA, static pages, shell, inspector |
| apps/ecommerce-cms-agentsam/backend/agentsam/ | AgentSam runtime modules | Routing, models, tools, prompts, files, conversations |
| packages/agentsam-workbench/ | Reusable miniAgentSam | Contextual composer, selection UI, attachments/capabilities |
| docs/RUNTIME-CONTRACTS-AGENTSAM.md | AgentSam app/UI/runtime SSOT | Model/effort, Work rail, plugins, tool breadcrumbs |
| apps/ecommerce-cms-agentsam/backend/cms/ | CMS authoring/publish | D1 + R2/KV + live editor integration |
...
Retired source note: root `admin-ui/`, `app/`, `src/`, and `public/admin/` are not runtime authority. Wrangler serves assembled `./dist/assets`; `npm run guard:boundaries` fails if retired source files reappear.
...
AgentSam asset sync:            npm run app:frontend:sync (also wired into dev/build/predeploy)
...
Step 2: Implement in fuelnfreetime — commerce, CMS, or admin app
...
[ ] 2026-09-23 — Continue model/effort + Work Project/Files/Plugins improvements against apps/ecommerce-cms-agentsam/frontend
```
...
*Created: 2026-07-06*
*Last updated: 2026-09-23 (apps/ecommerce-cms-agentsam is runtime authority; retired-root drift guarded)*
