...
Frontend:               app/frontend/ (AgentSam source) + admin-ui/ analytics SPA + static storefront in public/
Backend:                app/backend/ (AgentSam source) + legacy src/ domains still migrating
Frontend deploy:        Bundled in npm run deploy; app/frontend is synced into public/ asset output first
...
3. AgentSam source authority is app/frontend + app/backend. Do not restore implementation logic under public/admin/agentsam* or src/agentsam/*.
...
| src/admin/api.js | Admin API router | Existing router; AgentSam implementation resolves through app/backend |
| app/frontend/admin/agentsam/ | AgentSam frontend source | Chat / Work composer + thread UI |
| app/backend/admin/agentsam.js | AgentSam admin HTTP handler | Canonical handler implementation |
| app/backend/agentsam/ | AgentSam runtime modules | Routing, models, tools, prompts, files, conversations |
...
Legacy notes: `src/admin/agentsam.js` and `src/agentsam/*.js` are compatibility re-export bridges only. `public/admin/agentsam*` is generated by `npm run app:frontend:sync` because Wrangler still serves `./public` as the static asset directory.
...
[ ] 2026-09-07 — Finish model/effort picker + Work Project/Files/Plugins rail + @plugin token UI against app/frontend
```
...
*Created: 2026-07-06*
*Last updated: 2026-09-07 (AgentSam app/frontend + app/backend source authority)*
